...
for(const name of ['meow-cat','everythingisbroken']){
 ...
 for(const r of rows.filter(r=>r.name===name))html+=`<article><h3><a href="${r.model.includes('sunburst')?'https://openrouter.ai/':'https://replicate.com/'}${r.model}">${r.model}</a></h3>${r.width?`<a href="${r.key}.png"><img class="preview" src="${r.key}.png"></a><div><img class="small" src="${r.key}.png"><img class="mid" src="${r.key}.png"></div>`:esc(r.error??r.status)}<p>${r.cost==null?'Variable pricing':`~$${r.cost}/image`} · ${r.seconds?.toFixed(2)??'?'}s<br>${r.width??'?'}×${r.height??'?'} · alpha: ${r.alpha??'?'}</p></article>`;
 html+='</div>';
