...
const prompt = 'Reconstruct this tiny emoji as a genuinely crisp, high-quality finished image. Use the reference for exact composition, expression, proportions, pose and colors, but redraw the blurry edges and surfaces with clean coherent detail. Do not reproduce blur, pixelation, compression artifacts or sharpening halos. Keep the same character and the same crop, filling the square canvas to the same extent. Do not convert it into a sticker, add a border, change the pose or make a new scene. Do not invent readable lettering from illegible marks. For transparent areas use a uniform white background, never a checkerboard. The final should look like the original artwork before it was reduced to 30 pixels, with every edge sharp at full resolution.';
const configs = [
 ['google/nano-banana-pro',u=>({image_input:[u],prompt,resolution:'2K',output_format:'png',aspect_ratio:'1:1',allow_fallback_model:false}),.15],
 ['black-forest-labs/flux-2-max',u=>({input_images:[u],prompt,resolution:'2 MP',aspect_ratio:'1:1',output_format:'png',seed:42}),null],
 ['zsxkib/seedvr2',u=>({media:u,model_variant:'7b',sample_steps:1,seed:42,output_format:'png',apply_color_fix:true}),null],
];
const jobs=[];
for (const name of ['meow-cat','everythingisbroken']) {
 ...
 for(const [model,input,cost] of configs){
  ...
  if(existsSync(file)) result=JSON.parse(await fs.readFile(file));
   ...
   result=model==='zsxkib/seedvr2'?api('predictions',{version:schema.latest_version.id,input:input(uri)}):api(`models/${model}/predictions`,{input:input(uri)});
