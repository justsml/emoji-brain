...
Repository settings require approval for **all external contributors'** GitHub Actions runs, including returning contributors. To block merges on invalid emoji state, configure `check-emojis` as a required status check in branch protection. The live settings checked on 2026-09-09 had no required status checks, enabled code-owner review and stale-review dismissal, and allowed administrators to bypass protection. [CODEOWNERS](.github/CODEOWNERS) assigns review to the current maintainers. Approval to run a workflow is separate from approval to merge a PR. These GitHub settings must also be configured when forking the repository; files alone cannot enforce them.
