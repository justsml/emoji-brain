...
Checks print a compact terminal table and report prior metadata existence, missing files, SHA-256 changes, filesystem modification-date changes, actual image type, required fields, duplicate IDs/filenames, duplicate content, dimensions, and animated still availability. `--json` remains available for machine-readable output, while GitHub step summaries use Markdown. Invalid, stale, or hash-changed records exit with status 1. Filesystem modification-date changes are informational because timestamps change on checkout. Run an offline update to accept current image changes and refresh their recorded hashes and dates.
