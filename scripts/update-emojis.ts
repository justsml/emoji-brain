...
import { checkFailed, googleApiKey, recoveryGuidance } from './emoji-check-guidance';

try {
  const args = process.argv.slice(2).filter(a => a !== '--');
  if (args.includes('--help')) {
    console.log('Usage: pnpm update-emojis [--update[=none|all|changes]]\nDefault: changes. Converts images, ingests public/emojis/ingest, updates metadata, generates stills, validates, and rebuilds search. Live modes send selected images to Google and require GOOGLE_API_KEY, GEMINI_API_KEY, or GOOGLE_GENERATIVE_AI_API_KEY.');
  } else {
    ...
    if (mode !== 'none' && (process.env.CI || process.env.GITHUB_ACTIONS || !googleApiKey())) {
      ...
      throw new Error(`${process.env.CI || process.env.GITHUB_ACTIONS ? 'Live labelling is disabled in CI.' : 'A Google Gemini API key is required for live labelling. Set GOOGLE_API_KEY, GEMINI_API_KEY, or GOOGLE_GENERATIVE_AI_API_KEY in your local environment or .env.'}\n${recoveryGuidance(report)}`);
    }
