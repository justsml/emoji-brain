...
if(!['pending-human-approval','approved-promoted'].includes(manifest.status)||manifest.items.length!==manifest.count)throw Error('Invalid manifest');
...
for(const r of manifest.items){
 ...
 if(!['pending','approved'].includes(r.approval))throw Error('Unexpected approval '+r.file);
 if(r.approval==='approved'&&r.approvedSha256!==hash(candidate))throw Error('Stale approval '+r.file);
...
try{
 ...
 console.log(JSON.stringify({candidates:manifest.count,...result,hashes:'valid',approval:manifest.status,filter:'passed',background:'passed'}));
