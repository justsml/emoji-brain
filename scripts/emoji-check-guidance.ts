...
export function recoveryGuidance(report: Awaited<ReturnType<typeof checkEmojis>>) {
  ...
  if (report.pendingLabels.length) {
    if (process.env.CI || process.env.GITHUB_ACTIONS) {
      lines.push('Live labelling is disabled in CI. Configure a Google Gemini API key locally; do not add it to the PR or workflow.');
    } else if (googleApiKey()) {
      lines.push('A Google Gemini API key is configured locally (presence checked; validity is verified when labelling runs).');
    } else {
      lines.push('No Google Gemini API key is configured. Set GOOGLE_API_KEY, GEMINI_API_KEY, or GOOGLE_GENERATIVE_AI_API_KEY in your local environment or .env (Bun loads it), then rerun `pnpm update-emojis --update=changes`. Obtain a key from Google AI Studio. Do not commit the key.');
    }
